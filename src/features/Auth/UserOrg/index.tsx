...
import { selectOrganisation } from '@state/auth/slice'
  ...
  return (
    <Modal onClose={handleClose} show={show} blank narrow={view === 'user'}>
      <TransitionContainer showModal={show} hide={view !== 'user'} width="400px">
        <UserView onEditOrg={handleEditOrg} onSelectOrg={handleSelectOrg} />
      </TransitionContainer>
      {hasOrg && (
        <TransitionContainer showModal={show} hide={view !== 'org'} width="800px">
          <OrgView onBack={() => setView('user')} selectedOrg={selectedOrg} />
        </TransitionContainer>
      )}
    </Modal>
  )
